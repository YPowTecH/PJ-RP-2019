...
//    G_Printf("serverTime >>>>>\n" );
  ...
  //By PowTecH - Ads
  if (client->sess.lastAd <= level.time) {
    if (Q_stricmp(g_adMessage.string, "") != 0) {
      trap_SendServerCommand(ent - g_entities, va("print \"^5[^7Ad^5] %s\n\"", g_adMessage.string));
      client->sess.lastAd = level.time + g_adInterval.integer;
    }
  }
...
//  BotTestAAS(ent->r.currentOrigin);
  ...
  if (client->ps.forceKickFlip)
  {
      ...
      (!ent->client->ps.duelInProgress || ent->client->ps.duelIndex == faceKicked->s.number))
    {
		...
		if (ent->client->sess.queueTeam == faceKicked->client->sess.queueTeam && !g_friendlyFire.integer) {
      ...
      else if ( faceKicked && faceKicked->client && faceKicked->health && faceKicked->takedamage )
