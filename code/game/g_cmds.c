...
static void Cmd_Say_f( gentity_t *ent, int mode, qboolean arg0 ) {
  ...
  if (!strcmp(p, "!queue") || !strcmp(p, "!que") || !strcmp(p, "!q")) {
    //need to actually break up the queuepop into min and seconds
    trap_SendServerCommand(ent - g_entities, va("print \"^5[^7In Queue: '^5%i^7' Starting in: '^5%i^7'^5]^7\n\"", level.queueCount, level.queuePop));
    Cmd_Queue_f(ent);
  }
...
void Cmd_JoinQueue_f(gentity_t *ent) {
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    if (level.inQueue[i] == ent->s.number) {
      trap_SendServerCommand(ent - g_entities, va("print \"^1[^7You are already in queue^1]^7\n\""));
      return;
    }
...
void Cmd_LeaveQueue_f(gentity_t *ent) {
  ...
  //not in queue
  if (!found) {
    trap_SendServerCommand(ent - g_entities, va("print \"^1[^7You were not in queue^1]^7\n\""));
    return;
  }
  //found you
  else {
    trap_SendServerCommand(ent - g_entities, va("print \"^2[^7You left queue^2]^7\n\""));
  }
